...
for i in range(0, ru):
    ...
    curr = runtime[0]
    peak = runtime[1]
    lst.append([sys.argv[1], 'Beta', i, ep, curr, peak])
m = sys.argv[1]
if m == 'base':
    rollout = 5
    min_atoms = 8
...
for i in range(0, ru):
    ...
    curr = runtime[0]
    peak = runtime[1]
    lst.append([sys.argv[1], 'SubgraphX', i, ep, curr, peak])
print('GNN!')
if sys.argv[1] == 'base':
    lr = 1e-5
    seed = 10
...
for i in range(0, ru):
    ...
    curr = runtime[0]
    peak = runtime[1]
    lst.append([sys.argv[1], 'GNN', i, ep, curr, peak])
df = pd.DataFrame(lst, columns=['Data', 'Explainer', 'Experiment', '# Epochs', 'Current Memory', 'Peak Memory'])

df.to_csv(f'mem/{sys.argv[1]}Memory.csv')