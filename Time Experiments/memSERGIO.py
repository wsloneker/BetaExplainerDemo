...
for sparse in sparsity:
    ...
    grdta = torch_geometric.data.Batch.from_data_list(graph_data)
    graph_data = DataLoader(grdta, batch_size=1)
    graph_data1 = DataLoader(grdta, batch_size=num_graphs)
    print(graph_data)
    dataset = graph_data
    ...
    for i in range(0, ru):
        ...
        if sparse == 25:
            ...
            beta = 0.65
        tracemalloc.start()
        expbeta = BetaExplainer(model, graph_data1, G, torch.device('cpu'), 2000, alpha=alpha, beta=beta)
        expbeta.train(ep, lr)
        runtime = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        print(f'Beta, Sparsity: {sys.argv[1]}, Memory: {runtime}')
        lst.append([sys.argv[1], 'BetaExplainer', i, ep, runtime])
        if int(sys.argv[1]) == 25:
            ...
            sample_num = 9
        tracemalloc.start()
        expsubgraphx = SubgraphX(model, rollout = rollout, min_atoms = min_atoms, c_puct = c_puct,
                              expand_atoms = expand_atoms, sample_num = sample_num)
        runtime = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        print(f'SubgraphX, Sparsity: {sys.argv[1]}, Memory: {runtime}')
        lst.append([sys.argv[1], 'SubgraphX', i, ep, runtime])
        print('GNN!')
        ep = 25
        lr = 0.00001
        tracemalloc.start()
        ...
        runtime = tracemalloc.get_traced_memory()
        tracemalloc.stop()
        print(f'GNN, Sparsity: {sys.argv[1]}, Memory: {runtime}')
        lst.append([sys.argv[1], 'GNNExplainer', i, ep, runtime])
    df = pd.DataFrame(lst, columns=['Data', 'Explainer', 'Experiment', '# Epochs', ' Memory'])
    df.to_csv(f'mem/{sys.argv[1]}Runtimes.csv')